                          ...
                          children: <Widget>[
                            Container(
                              ...
                              child: Row(
                                children: <Widget>[
                                 ClipOval(
                                        child: doctors[index]['DocPic'] != ""
                                            ? Image.network(
                                                doctors[index]['DocPic'],
                                                width: 80,
                                                height: 80,
                                                fit: BoxFit.cover,
                                              )
...
/////////////////////////////////////////////////////////////////// GET doctors for ur appointementS
    ...
    doctorsref
        .where('patientUID',
            isEqualTo: FirebaseAuth
                .instance.currentUser!.uid) //.add(Duration(hours: 1 ))
        .where('date',
  ...
  var sref = FirebaseFirestore.instance
      ...
      .where('patientUID', isEqualTo: FirebaseAuth.instance.currentUser!.uid)
      .where('date',
          isGreaterThan: Timestamp.fromDate(
              DateTime.now().add(Duration(hours: 1, minutes: 10))))
      .orderBy('date')
  ...
  _cancelAppointment() async {
    ...
    notref
        .where('patientUID', isEqualTo: FirebaseAuth.instance.currentUser!.uid)
        .where('doctorUID', isEqualTo: doctors[iii]['doctorUID'])
    ...
    var ref = FirebaseFirestore.instance.collection('Patuser');
    await ref
        .doc(FirebaseAuth.instance.currentUser!.uid)
        .collection('ClickDoc')
        .doc( doctors[iii]['doctorUID']) //the doctor clicked uid
  ...
  void _onLoading() {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          insetPadding: const EdgeInsets.fromLTRB(120, 0, 120, 0),
          content: Row(
